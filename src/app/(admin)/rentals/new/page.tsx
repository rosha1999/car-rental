...
function NewRentalContent() {
  ...
  const [rentalType, setRentalType] =
    useState<(typeof RENTAL_TYPES)[number]>("DAILY");
  ...
  const pricing =
    ...
    new Date(endDate) > new Date(startDate)
      ? calculatePrice({
          ...
          customPrice: selectedCar.customPrice,
        })
      : null;

  const totalPrice = Math.max(
    0,
    (pricing?.basePrice ?? 0) - discount
  );
            ...
            >
              ...
              {cars.map((c) => (
                <option key={c.id} value={c.id}>
                  {c.name} ({c.plate}) — {formatIQD(c.dailyPrice)}/day
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="card space-y-3">
            ...
            <div className="bg-blue-50 p-4 rounded-xl space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                <span>Deposit</span>
                <span>
                  {formatIQD(selectedCar?.deposit ?? 0)}
                </span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="label">Discount (IQD)</label>
                <input
                  type="number"
                  min="0"
                  className="input"
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setPaymentMethod(
                      e.target.value as (typeof PAY_METHODS)[number]
                    )
                  }
                >
                  {PAY_METHODS.map((m) => (
            ...
            <button
              type="submit"
              disabled={saving || !customerId || !carId}
